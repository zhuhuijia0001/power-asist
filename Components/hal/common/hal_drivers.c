...
{	
	...
	/* LCD */
	InitLCD((Orientation)g_screenAngle);
	ClearScreen(BLACK);
  ...
  /* HID */
	...
	if (DetectFUSB302())
	{
		uint8 id = GetFUSB302DeviceID();
		TRACE("FUSB302 id:0x%02X\r\n", id);
...
uint16 Hal_ProcessEvent( uint8 task_id, uint16 events )
{
	...
	if ( events & SYS_EVENT_MSG )
	{
		...
		while (msgPtr)
		{
			...
			if (*msgPtr == FUSB302_MSG_ID)
			{
				ProcessFUSB302Interrupt();
			}
	...
	//quick charge 
	if (events & QC_CHECK_SUPPORT_EVT)
	{
		CheckQC20Supported();
		
		return events ^ QC_CHECK_SUPPORT_EVT;
	}

	//detect d
	if (events & DETECT_CHECK_EVT)
	{
		CheckDType();

		return events ^ DETECT_CHECK_EVT;
	}

	//auto detect
	if (events & AUTO_DETECT_CHECK_EVT)
	{
		AutoDetectTimeout();
		
		return events ^ AUTO_DETECT_CHECK_EVT;
	}
	
	//usb pd
	if (events & USB_PD_EVENT)
	{
		ProcessUsbPdTimeout();
		
		return events ^ USB_PD_EVENT;
	}

	//sample w & a
	if (events & SAMPLE_A_W_EVENT)
	{
		AccumulateLoadWhAndAh();
		
		return events ^ SAMPLE_A_W_EVENT;
	}
	
	//keys
	uint8 index;
	for (index = KEY_FIRST; index <= KEY_LAST; index++)
	{
		if (events & (HAL_KEY_EVENT << index))
		{
#if (defined HAL_KEY) && (HAL_KEY == TRUE)
			/* Check for keys */
			HalKeyPoll(index);
#endif
			return events ^ (HAL_KEY_EVENT << index);
		}
	}
